...
class Enemy(Entity):
    def __init__(self, monster_name, pos, groups, obstacle_sprites, damage_player, trigger_death_particles, add_xp):
        ...
        # sound
        self.death_sound = p.mixer.Sound('zelda/audio/death.wav')
        self.hit_sound = p.mixer.Sound('zelda/audio/hit.wav')
        self.attack_sound = p.mixer.Sound(monster_info['attack_sound'])
    ...
    def get_player_distance(self, player):
        enemy_vec = p.math.Vector2(self.rect.center)
        player_vec = p.math.Vector2(player.rect.center)
        distance = (player_vec - enemy_vec).magnitude()

        if distance > 0:
            direction = (player_vec - enemy_vec).normalize()
        else:
            direction = p.math.Vector2()
